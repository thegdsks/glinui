...
function rehypeComponent() {
  return async (tree) => {
    visit(tree, "element", (node) => {
      ...
      if (node.tagName === "ComponentPreview" || node.tagName === "BlockPreview") {
        const name = getNodeAttributeByName(node, "name")?.value;
        if (!name) {
          return null;
        }
...
function getNodeAttributeByName(node, name) {
  return node.properties?.attributes?.find((attr) => attr.name === name);
}
...
var computedFields = {
  ...
  image: {
    type: "string",
    resolve: (post) => `${process.env.NEXT_PUBLIC_APP_URL}/api/og?title=${encodeURI(post.title)}`
  },
  slug: {
    type: "string",
  ...
  structuredData: {
    ...
    resolve: (doc) => ({
      ...
      image: doc.image,
      url: `https://glinui.com/${doc._raw.flattenedPath}`,
      author: {
        "@type": "Person",
        name: doc.author,
...
//# sourceMappingURL=compiled-contentlayer-config-YTDENEXD.mjs.map
