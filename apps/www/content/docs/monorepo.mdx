---
title: Monorepo
description: Using glinui/ui components and CLI in a monorepo.
---

Until now, using glinui/ui in a monorepo was a bit of a pain. You could add
...
```bash
npx glinui@canary add [COMPONENT]
```

The CLI will figure out what type of component you are adding and install the
correct files to the correct path.

For example, if you run `npx glinui@canary add button`, the CLI will install the button component under `packages/ui` and update the import path for components in `apps/web`.

If you run `npx glinui@canary add login-01`, the CLI will install the `button`, `label`, `input` and `card` components under `packages/ui` and the `login-form` component under `apps/web/components`.
