...
```json title="registry-item.json" showLineNumbers
{
  ...
  "files": [
    ...
    {
      "path": "registry/tailwind/hello-world/use-hello-world.ts",
      "type": "registry:hook"
    }
  ]
}
...
You can see the JSON Schema for `registry-item.json` [here](https://glinui.com/schema/registry-item.json).
...
- For `glinui/ui` registry items such as `button`, `input`, `select`, etc use the name eg. `['button', 'input', 'select']`.
...
```json title="registry-item.json" showLineNumbers
{
  "files": [
    ...
    {
      "path": "registry/tailwind/hello-world/.env",
      "type": "registry:file",
      "target": "~/.env"
    }
  ]
...
By default, the `glinui` cli will read a project's `components.json` file to determine the target path. For some files, such as routes or config you can specify the target path manually.
