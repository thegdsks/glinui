...
async function captureScreenshots() {
  ...
  const blocks = blockIds.filter((block) => {
    ...
    const lightPath = path.join(
      REGISTRY_PATH,
      `styles/tailwind/${block}-light.png`
    )
    const darkPath = path.join(
      REGISTRY_PATH,
      `styles/tailwind/${block}-dark.png`
    )
  ...
  for (const block of blocks) {
    const pageUrl = `http://localhost:3333/view/styles/tailwind/${block}`
    ...
    for (const theme of ["light", "dark"]) {
      const screenshotPath = path.join(
        REGISTRY_PATH,
        `styles/tailwind/${block}${theme === "dark" ? "-dark" : "-light"}.png`
      )
