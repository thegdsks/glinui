...
import {
  ...
  CollapsibleContent,
  CollapsibleTrigger,
} from "@/registry/tailwind/ui/collapsible"
import {
  ResizableHandle,
  ResizablePanel,
  ResizablePanelGroup,
} from "@/registry/tailwind/ui/resizable"
...
import { Tabs, TabsList, TabsTrigger } from "@/registry/tailwind/ui/tabs"
import {
  ToggleGroup,
  ToggleGroupItem,
} from "@/registry/tailwind/ui/toggle-group"

import {
  registryItemFileSchema,
  registryItemSchema,
} from "../../../packages/glinui/src/registry/schema"
...
function BlockViewerProvider({
  ...
  const [style, setStyle] =
    React.useState<BlockViewerContext["style"]>("tailwind")
      ...
      <div className="ml-auto hidden items-center gap-2 md:flex">
        ...
        <div className="flex h-7 items-center gap-1 rounded-md border p-[2px]">
          <Button
            ...
            size="sm"
            onClick={() => {
              copyToClipboard(`npx glinui@latest add ${item.name}`)
            }}
          >
            {isCopied ? <Check /> : <Terminal />}
            <span className="hidden lg:inline">npx glinui add {item.name}</span>
          </Button>
        </div>
