...
const REGISTRY_URL = "https://glinui.com/r"

const server = setupServer(
  http.get(`${REGISTRY_URL}/styles/tailwind/button.json`, () => {
    return HttpResponse.json({
      ...
      files: [
        {
          path: "registry/tailwind/ui/button.tsx",
  ...
  http.get(`${REGISTRY_URL}/styles/tailwind/card.json`, () => {
    return HttpResponse.json({
      name: "card",
      type: "registry:ui",
      dependencies: ["@radix-ui/react-slot"],
      files: [
        {
          path: "registry/tailwind/ui/card.tsx",
...
describe("fetchRegistry", () => {
  it("should fetch registry data", async () => {
    const paths = ["styles/tailwind/button.json"]
  ...
  it("should use cache for subsequent requests", async () => {
    ...
    server.use(
      http.get(`${REGISTRY_URL}/styles/tailwind/button.json`, async () => {
  ...
  it("should handle multiple paths", async () => {
    const paths = ["styles/tailwind/button.json", "styles/tailwind/card.json"]
    const result = await fetchRegistry(paths)
